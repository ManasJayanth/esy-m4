...
`esy-m4` is package, [`m4`](https://www.gnu.org/software/m4/), packaged for [`esy`](https://esy.sh/).
...
`esy-m4` can be used from both NPM and directly from Github.

### From NPM 

`esy-m4` is deployed on NPM can be found
[here](https://www.npmjs.com/package/TODO).

You can simply run `esy add esy-m4` to install it, or specify it in
...
```diff
  "dependencies": {
+   "esy-m4": "esy-packages/esy-m4#<commit hash>"
  }
```
